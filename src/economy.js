...
function advisorFor(broke, unemp, pop, popCap, happiness, demand, extra) {
  ...
  if ((extra.plants || 0) >= 1 && (extra.cisterns || 0) < 1) {
    ...
    if (happiness < 38) return 'Mood is low. Tap this chip for a water tower — dry lots sour the town.';
...
export function tick(city) {
  ...
  note(
    city,
    'tipMains',
    city.tickCount === 56 && plants < 1 && raisingPlants < 1 && (util.towers || 0) < 1 && raisingCisterns < 1 && sewerWorks < 1 && raisingWorks < 1,
    'Houses run on wells and kerosene. A plant, a tower, and a works keep a real town alive.',
  );
